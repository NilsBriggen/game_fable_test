...
| Village ground: beaten earth with a worn cart track through it, grass creeping back at the edges, dung/rain hollows | Ground appearance of every settlement pad and of the roads between them (`src/world/look/splat.ts`, `src/world/terrainMaterial.ts`) | world-look builder | No charter records what the Dorfplatz of Altdorf or Schwyz was *paved* with, and for a 13th–14th c. Alpine village the honest answer is "nothing": stamped earth, stone where the stone was already there, mud where the cattle stood. Cobbled squares and drained lanes belong to the later Swiss towns, not to Uri and Schwyz in 1291–1315. The cart-track width (~3 m) is that of a two-wheeled Saumkarren track, not a Roman road. |
| Tree-species mix by altitude: spruce ≫ fir > beech > larch below ~2/3 of the tree line, larch increasing with height, mountain pine (Bergföhre) in the last belt under it, nothing above game h 355 (real ~1500 m a.s.l.) | Procedural forest composition (`src/world/vegetation.ts`) | world-look builder | LORE §3 fixes the tree line; the belt order is the standard Central-Alpine montane/subalpine sequence (Fichten-Tannen-Buchenwald → Lärchen-Arvenstufe → Legföhrengürtel). Which individual tree stands where is procedural and carries no historical claim. |
| Forest clearings ("glades") and the density of the wood | Low-frequency noise field thinning the canopy in patches (`src/world/vegetation.ts`) | world-look builder | Medieval Waldstätte woodland was worked — pastured, coppiced, charcoal-burned — not primeval closed canopy, so patchy density with walkable clearings is closer to the period than a uniform plantation. The specific pattern is procedural. |
| Character dress rendering conventions (`src/world/characters/looks.ts`): men in knee-length cotte + hose + turnshoes with hood (Gugel), linen coif, round cap or felt hat; married women in ankle gown, apron, veil + wimple; Confederate militia in quilted gambeson + Eisenhut over a linen arming cap; Habsburg troops in mail + red-white-red surcoat, sergeants/knights in bascinet + aventail; monks tonsured in dark habit | Visual dress conventions per archetype | character-art builder | All attested 13th/14th-c. Alemannic dress; nothing here is named in §7 beyond the armour list, but none of it is an anachronism (the liripipe hood, coif and wimple are ubiquitous in c. 1300 iconography, the Bindenschild colours are the Austrian ducal arms). Skin tones, hair colours and the six face variants per archetype are the builder's choices, not lore. |
| Village and farmstead prop set (`src/world/models/*`): the Spycher granary on mushroom-capped staddle stones (`granary`), a trestle market stall with an awning (`market.stall`), stacked firewood (`woodpile`), a hollowed-log water trough (`trough`), a ruined masonry wall stub (`ruin.wall`), and the dressing added to buildings that layout.ts already places — firewood and a bench at every farmhouse, a trough and cobbled apron at every well, a stall and a barrel at the village tavern | Building/prop forms and their village dressing | buildings-and-props builder | All are ordinary attested Alpine/Alemannic rural building practice rather than new lore: the Blockbau on a drystone plinth with a Laube gallery, board gable and stone-weighted shingle roof; the Stadel raised on stone piers; the Spycher on staddle stones (mouse caps); Letzi drystone barriers; the Bergfried with a raised entrance (Hocheingang) and timber stair. Individual proportions, the number of courses, and which prop stands where are the builder's choices. Nothing on the market stall is post-Columbian (bread, cheese, a barrel — no potatoes or maize, LORE §7). |
| Town house and tavern forms composed from the CC0 Medieval Village MegaKit (`src/world/models/townhouse.ts`): rubble-stone ground storey, limewashed upper storey with half-timber (Fachwerk) panels and board shutters, a jettied Laube gallery on the tavern, a round-tile (Mönch-Nonne / Hohlziegel) roof with a dormer on the Luzern/Zug/Sarnen town house and a stone-weighted shingle roof on the village tavern; Poly Haven scans for the well bucket, tavern table/stools/barrel/crate/lantern, chopping-block axe, fire ring and granary ladder | Building/prop forms | buildings-and-props builder (wave 2) | A stone ground storey under a timber/plaster upper storey and clay-tile roofs are attested for late-13th-c. Alemannic towns (Luzern, Zug), while the Waldstätte village tavern (Sust/Taverne on the Gotthard route) keeps the Alpine shingle roof; the exact panel patterns, shutter colours and which prop stands where are the builder's choices. No object is post-Columbian; the window "glass" is the dark oiled-parchment/bull's-eye look, not clear plate. |
| `axen-fluh`, `axen-wand`, `ingenbohl-shore`, `brunnen-east` | Gazetteer waypoints on the Urnersee's eastern shore (no settlements, no NPCs) | integrator | The Axen path's Sisikon→Brunnen leg was a straight chord through open water; the real shore path hugs the Axen cliffs. Names describe real features (Axenfluh, Ingenbohl), not invented places. |
| `ruetli-steig` | Gazetteer waypoint 50 m above the Rütli where the Seelisberg footpath ends | integrator | The mule-track grade limit could not bring a road down to the meadow, and a road ending on the Rütli made the mandated meadow a road cut; descriptive name, no invented place. |
| `quest.marchenstreit` restraint/negotiated journals (`raid` onEnter, `negotiate-success`/`negotiate-fail`) | Path-specific journal memory for the Einsiedeln outcome (plunder vs parley-kept vs parley-failed) | quest builder | The Marchenstreit escalation and excommunication are H (§1/§6); which words were spoken at the gate and how the raiders behaved is undocumented detail, i.e. I dressing on an H event. The ban falls in all paths, as attested. |
| `dlg.muster-recruit` restraint variants, `dlg.abt-johannes` `ch2-other` variants, `quest.muster-1315` `recruit` journal | Downstream readbacks of the Einsiedeln choice during the muster year | quest builder | Reactions of invented-dressing recruiting rounds and of the H Abbot (§5) to the player's documented-in-game conduct; no new places, persons, or dates. |
| `gessler-hat-choice=walked-past` Attinghausen variant; Stauffacher (`fought`) and Melchtal (`watched`) hat callbacks | NPC memory of the Altdorf hat choice | quest builder | The hat is L (§1); who remembers what about the player's conduct in the square is I dialogue dressing. Attinghausen/Stauffacher names H, Melchtal L per §5 — no status change. |
| `dlg.wilhelm-tell` `campfire-eve` / `campfire-after` | Two staged Tell companion beats (eve of the Hohle Gasse; after Gessler's death) | quest builder | Tell wholly L (§5); campfire words put in his mouth are I, confined to his existing companion window (Tellsplatte → der-hut epilogue). No new claims about the tradition. |
| `dlg.muster-retry`, `quest.muster-1315` `sattel-retry`, flag `morgarten.retry` | Recovery beat + routing for the Morgarten second pass | quest builder | The retry loop itself is existing machinery (quest-failed handler); the survivors-at-Sattel scene is I dressing. No history touched — Morgarten's H outcome (15 Nov 1315, Leopold escapes) is unchanged. |
| Procedural music beds (`tavern`/`church`/`battle`/`explore`/`morgarten`/`title`) + region ambience (`lake`/`mountain`/`village`/`church`) + SFX (clash/twang/step/fanfare/lament) in `src/ui/audio.ts`; `{music: 'music.*'}` quest effects on the Act 1 spine | Complete music/sound layer for Act 1 | quest builder | All melodies are short original phrases in D Dorian / G Mixolydian (evocation, not reconstruction); no historical tune is claimed or quoted. Tavern/church/battle associations are dramatic convention, not a claim about period performance practice. |
| `quest.der-saeumer` `crossing` var (`clean`/`rough`) → `reward-clean`/`reward-rough` stages; `dlg.saeumer-crossing` ledge-fight branch (`ledge-fight`/`ledge-held`/`ledge-loss`) with mule/salt loss stakes | Scaled Säumer reward for the Teufelsbrücke crossing (full purse + rep only on a clean crossing) | quest builder | The bridge (H, c. 1220–30) and Säumer cooperatives (H) are untouched; how the player's train fares on the arch, and what Niklaus pays for it, is undocumented detail, i.e. I dressing on an H crossing. |
| `quest.alpstreit` `inspect`/`examine` stages + `dlg.alpstreit-inspect` (`inspected=keen`/`glance` var, read back as a `hearing` variant) | Inspection beat on the disputed slope above Sattel (Steinerberg marker) before the letzi ruling | quest builder | The Bundesbrief arbitration clause (H mechanic) and the Sattel/Arth places (H) are untouched; walking the slope to read fresh-turned boundary stones is I dressing on attested boundary practice (annual beating of the bounds). No new places or persons. |
| `quest.fischer-von-gersau` `freedom`/`tribute` stages + `dlg.fischer-gersau-return`; flag `gersau-toll-owed` on the tribute path | Distinct success/fail journals plus a return-to-Uli beat; the failed path leaves a standing toll-debt flag for later readback | quest builder | Gersau's free-village status (H) and Habsburg road-toll practice (H in kind) are untouched; which words were spoken at the Brunnen quay and what Uli makes of them is I dressing. The toll-man Konrad Niederberger is himself an existing I NPC. |
| `quest.drache-vom-pilatus` `cache` stage + `dlg.drache-pilatus-cache` (discover + marked-cache + salt-sack item-grant beat at the Fräkmüntegg marker); Trudi Meier kept as giver with Engelberg-brother line | Physical salt-cache beat instead of a pure dialogue grant; Trudi kept as giver (lore-sweep sanctioned) with the monk folded in as her source | quest builder | The Pilatus "dragon" stays explicitly folk legend (lammergeier + smuggler, no monster — L per §6); untaxed Gotthard salt is H trade, and Fräkmüntegg is a real alp name (§4). No new NPC invented: the Engelberg brother is an unnamed figure in Trudi's telling. |
| `quest.schuetzenkoenig` bracket heats (`heats`/`talk`/`shoot` nodes; fixed seeded rival scores for Sepp, Ruedi, Burkhard in dialogue text) + `placing` var (`crown`/`placed`/`beaten`/`last`) | Bracket-round contest: two named rival scores stand before the player's single roll (DC 18 for the crown, DC 13 to place) | quest builder | Shooting contests are I for this date (§6); Sepp, Ruedi and the Burkhard lead are unnamed-invented heats, not new lore persons — fixed scores in dialogue text, no new RNG machinery. |
| `quest.bad-zu-wolfenschiessen` `quiet`/`exposed` stages + `dlg.bad-wolfenschiessen-clerk`; flag `wolfenschiessen-clerk-coming` on the slow path | Dawn timer pressure (`advanceTime` on the hide) with a too-slow branch: a clerk-from-Sarnen consequence hook and readback beat | quest builder | The bath-house killing is L told as a done thing (§6); whether the hiding was quick or sloppy, and what Jost fears from Landenberg's clerks (the Vogt-Schreiber apparatus, §5), is I dressing. No H outcome touched. |
